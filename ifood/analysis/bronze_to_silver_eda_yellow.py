...
# MAGIC %md
# MAGIC ## 1. Análise Táxis Amarelos (nyc_taxi.bronze.tb_bronze_yellow_taxi_trips)
...
df_yellow = spark.table("nyc_taxi.bronze.tb_bronze_yellow_taxi_trips")
display(df_yellow)
...
display(df_yellow.groupBy(col('year_month_file')).agg( \
...
## Análise total
display(df_yellow.select(count('*').alias('total_records')))
...
print(f"Quantidade de colunas: {len(df_yellow.columns)}")
...
display(df_yellow.filter((col('tpep_pickup_datetime').isNull()) | (col('tpep_dropoff_datetime').isNull())))
...
print(f"Quantidade de linhas geral: {df_yellow.count()}")
print(f"Quantidade de linhas duplicadas: {df_yellow.dropDuplicates().count()}")

# Utilizando como chave os dados de vendor id, dropoff e pickup para remover duplicados
print(f"Quantidade de linhas duplicadas por chave composta Vendor + pickup + dropoff: {df_yellow.dropDuplicates(['VendorID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime', 'trip_distance', 'PULocationID', 'DOLocationID']).count()}")
...
janela_duplicados = Window.partitionBy(
    "VendorID", "tpep_pickup_datetime", "tpep_dropoff_datetime", "trip_distance", "DOLocationID", "PULocationID"
)
...
).orderBy("VendorID", "tpep_pickup_datetime", "tpep_dropoff_datetime", "trip_distance", "DOLocationID", "PULocationID")
...
print(f"Quantidade de registro com drop duplicates depois do filtro: {df_visualizacao_final.filter((col('trip_distance') > 0) & (col('total_amount') > 0)).dropDuplicates(['VendorID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime', 'trip_distance', 'PULocationID', 'DOLocationID']).count()}")
...
display(df_analise_duplicados_2.filter(F.col("total_aparicoes") > 1).orderBy("VendorID", "tpep_pickup_datetime", "tpep_dropoff_datetime", "trip_distance", "total_amount"))

# COMMAND ----------

## contando o número de registros por passenger count
display(df_yellow.groupBy('passenger_count').count().orderBy(col('passenger_count').desc()))
...
# MAGIC
# MAGIC  - As colunas **VendorID**, **trip_distance**, colunas de localização e data, além de colunas de pagamento exceto airport_fee e congestion_surchage estão preenchidas com valores por completo (16.186.386, volumetria completa da tabela)
# MAGIC  - Maio de 2023 foi o mês com mais viagens reportadas no período analisado
# MAGIC  - As colunas que possuem valores nulos apresentam sempre a mesma volumetria de dados: 15.757.721 (delta de ~428k viagens, 2,6% da volumetria total da base)
...
# MAGIC  - O valor máximo de distância percorrida em **trip_distance** é superior a 300k milhas, além de termos viagens com distância zerada.
# MAGIC  - Temos 250k casos onde, pela chave de **VendorID**, datas de pickup e drop off, **trip_distance** e as localizações de pickup e drop_off temos dados duplicados. Parte destes casos apresentam valores diferentes, porém, boa parte apresenta valores de **total_amount** com um registro positivo e outro negativo. Desta forma, estes duplicados deveram ser eliminados. Para isto, temos somente 4 casos onde temos duplicados pelas chaves com total_amount positivos e diferentes. Dois destes casos são registros que começaram e terminaram na mesma hora (provavelmente serão eliminados). Como os outros 2 podem ser uma coincidência, não será necessário, para este caso, a remoção por regra de duplicados (as outras regras serão suficientes.)
# MAGIC  - 
# MAGIC
...
print(f"Quantidade de linhas com passenger count nulo: {df_yellow.filter(col('passenger_count').isNull()).count()}")
print(f"Quantidade de linhas nulas com total_amount negativo: {df_yellow.filter((col('passenger_count').isNull()) & (col('total_amount') < 0)).count()}")

### Somente 172 itens com total_amount negativo e passenger_count nulo

# Quantidade SOMENTE de total_acount_nulo
print(f"Quantidade de linhas com total_amount negativo: {df_yellow.filter(col('total_amount') < 0).count()}")

# COMMAND ----------

display(df_yellow.filter(col('passenger_count').isNull()))

# COMMAND ----------

# Distribuição de valores nulos de total amount por partição + de valores nulos
display(df_yellow.filter(col('total_amount') <= 0).groupBy(col('year_month_file')).agg(count('*').alias('total_records')))
display(df_yellow.filter(col('passenger_count').isNull()).groupBy(col('year_month_file')).agg(count('*').alias('total_records')))

# COMMAND ----------

# Verificando os valores nulos de passenger_count e os outros distintos
display(df_yellow.filter(col('passenger_count').isNull()).groupBy('VendorId', 'RatecodeID', 'store_and_fwd_flag', 'congestion_surcharge', 'airport_fee').count().alias('total_records'))

## Vários casos de Vendor 2 tem valores de passageiro nulo, poucos casos de 6


# COMMAND ----------

## Verificando contagem de passenger_count distintos e como estão distribuídos os dados de 
display(df_yellow.groupBy('passenger_count').agg( \
    count('*').alias('count'), \
...
df_datas_silver_base = df_yellow.select(
    ...
    hour(col("tpep_pickup_datetime")).alias("hora_do_dia_pickup"),
    hour(col("tpep_dropoff_datetime")).alias("hora_do_dia_dropoff"),
    ...
    datediff(col("tpep_dropoff_datetime"), col("tpep_pickup_datetime")).alias("duracao_dias"),
    
    # E. Ano e mês do pickup e do dropoff (Formato YYYY-MM)
    date_format(col("tpep_pickup_datetime"), "yyyy-MM").alias("ano_mes_pickup"),
    date_format(col("tpep_dropoff_datetime"), "yyyy-MM").alias("ano_mes_dropoff")
)
...
### Neste caso, temos 99% das viagens ocorrendo dentro de um dia. Casos com mais de um dia de duração somam 66 casos, onde grande pare do valor pago e máxima distância registrada não é significativa. Sendo assim, a conclusão é que estes 66 registros serão retirados de cara
...
# 91,4% dos casos com trip_distance = 0 tem total_amount positivo. 11% de todos os casos com trip_distance = 0 são casos de passageiros nulos. Casos de trip distance representam entre 1 e 1,5% dos casos do dataset. Para termos de análises, vamos desconsiderar estas viagens.
...
### Temos dados com viagens de duração negativa ou até com duração <= 0
...
##### Temos grande parte dos casos armazenados onde a diferença entre os meses é nula, com alguns casos de 1. Vamos considerar o pickup e remover casos onde a diferença de meses entre o arquivo e a data de pickup é superior a 0, ou seja, itens que não são do mesmo mês
...
# MAGIC - Remover viagens onde a duração da corrida é >= 2 dias
...
df_clean_logica = (df_datas_silver_base
    ...
    .filter(
        ...
        (F.col("duracao_dias") < 2) &                            # Remove duração >= 2 dias
...
# Utilizando como chave os dados de vendor id, dropoff e pickup para remover duplicados
print(f"Quantidade de linhas duplicadas por chave composta Vendor + pickup + dropoff: {df_silver_pre_outliers.dropDuplicates(['VendorID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime', 'trip_distance', 'PULocationID', 'DOLocationID']).count()}")
...
# MAGIC
# MAGIC
# MAGIC - Avaliando as variáveis **trip_distance**, **total_amount** e **duracao_minutos_floor**, vemos uma distribuição de calda longa, onde até P99 os valores fazen sentido dentro de em uma viagem e valores entre P99 e P100 apresentam outliers muito altos.
# MAGIC - As comparação entre as medianas das três variáveis variam entre **3 e 4 vezes** o valor de P99. Porém, existem valores dentro destes casos que podem ser reais. Exemplos são viagens de aeroporto ou distância maiores que 20 milhas, por exemplo, que podem tanto custar mais de USD 100 e também durar mais de 1 hora (todos valores aproximados de P99). Assim, calculamos valores de **tarifa_por_milha** e **velocidade_mph**.
# MAGIC - Para estes dados, a distribuição mostra que P1, as medianas (P50) e P99 destes valores são, aproximadamente:
# MAGIC -- **tarifa_por_milha** ($/mi): 4,3 ; 11,07; 36,06
# MAGIC -- **velocidade_mph**: 3,16; 10; 35,96
# MAGIC
...
# (Caso o df_yellow ainda não tenha a coluna year_month_file, 
# ajustamos aqui para o script rodar. Exemplo: arquivo de Jan/2023)
# ==============================================================================
if "year_month_file" not in df_yellow.columns:
    df_yellow = df_yellow.withColumn("year_month_file", F.lit("2023-01"))

volume_original = df_yellow.count()
...
df_aux = (df_yellow
    ...
    .withColumn("duracao_segundos", F.unix_timestamp("tpep_dropoff_datetime") - F.unix_timestamp("tpep_pickup_datetime"))
    ...
    .withColumn("ano_mes_pickup", F.date_format("tpep_pickup_datetime", "yyyy-MM"))
    .withColumn("data_ref_arquivo", F.to_date(F.col("year_month_file"), "yyyy-MM"))
    .withColumn("data_ref_pickup", F.to_date(F.col("ano_mes_pickup"), "yyyy-MM"))
    .withColumn(
...
df_clean_logica = (df_aux.filter(
        ...
        (F.col("duracao_dias") < 2) &                            # Remove viagens de 2 dias ou mais
...
print(f"1. Volume Original (df_yellow): {volume_original:,}".replace(",", "."))
...
# MAGIC
# MAGIC - **Duração Irreal (Limites Superiores):** Remoção de viagens onde a duração da corrida é >= 2 dias.
# MAGIC
...
# MAGIC
# MAGIC Cortar os dados unicamente com base em tetos estatísticos absolutos resultaria na perda de dados reais. Viagens longas autênticas (como trajetos para aeroportos com mais de 20 milhas, durações superiores a 1 hora ou custos acima de USD 100) estariam em risco. Para contornar isso, a abordagem adotada foi a criação e análise de variáveis derivadas: Tarifa por Milha (tarifa_por_milha) e Velocidade (velocidade_mph).
# MAGIC
# MAGIC A distribuição estatística destas variáveis derivadas (P1, P50/Mediana e P99) demonstrou os seguintes patamares:
# MAGIC
# MAGIC - **Tarifa por Milha ($/mi)**: P1 = 4,30 | P50 = 11,07 | P99 = 36,06
# MAGIC
# MAGIC - **Velocidade (mph):** P1 = 3,16 | P50 = 10,00 | P99 = 35,96
# MAGIC
...
# MAGIC
# MAGIC *Regra Aplicada:* Será utilizado um fator de tolerância 2x sobre as pontas da curva. Serão removidos os valores que ficarem abaixo da metade do P1 (< 2,15 $/mi) e acima do dobro do P99 (> 72,12 $/mi).
...
# MAGIC
# MAGIC - **pickup_datetime** (Timestamp): Data e hora exata do início da viagem (renomeado de tpep_pickup_datetime).
# MAGIC
# MAGIC - **dropoff_datetime** (Timestamp): Data e hora exata do fim da viagem (renomeado de tpep_dropoff_datetime).
# MAGIC
