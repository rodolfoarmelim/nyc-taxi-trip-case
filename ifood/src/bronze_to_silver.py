...
# ==============================================================================
def apply_silver_transformations(spark, table_source: str, color: str, particoes: list):
    """
    ...
    # Início de cálculo de percentis por partição
    dfs_filtrados = []
    for particao in particoes:
        df_partition = df_metricas.filter(F.col("year_month_file") == particao)

        # 4. Cálculo Dinâmico de Percentis (Calculado isoladamente para a cor atual)
        limites = df_partition.approxQuantile(["tarifa_por_milha", "velocidade_mph"], [0.01, 0.99], 0.001)
        p1_tarifa, p99_tarifa = limites[0][0], limites[0][1]
        p1_vel = limites[1][0]

        # 5. Aplicação das Regras de Negócio (Expurgo de Outliers)
        df_partition_clean = df_partition.filter(
        ...
        dfs_filtrados.append(df_partition_clean)
    
    # Unindo os dataframes limpos por partição
    df_filtered = reduce(DataFrame.unionByName, dfs_filtrados)
...
# ==============================================================================
def process_bronze_to_silver(spark, particoes: list):
    """
