...
def create_table_gold_full_data(spark, df):
    """
    ...
    partition_list_dropoff_year_month = [linha["dropoff_time_year_month"] for linha in df_final_gold.select("dropoff_time_year_month").distinct().orderBy("dropoff_time_year_month").collect()]

    print(f"  -> [ESCRITA] Salvando partição/ões {partition_list_dropoff_year_month} na tabela Gold: {GOLD_TABLE_METADATA['full_analysis']['table']}")

    formatted_partitions = ", ".join([f"'{p}'" for p in partition_list_dropoff_year_month])

    save_as_table_delta(df_final_gold, "overwrite", GOLD_TABLE_METADATA["full_analysis"]["table"], "dropoff_time_year_month", formatted_partitions)


def create_table_gold_analysis_per_hour_per_month_full(spark, df):
    """
    ...
    df_gold_groupping = df.groupBy("dropoff_time_year_month", "dropoff_time_hour")\
        .agg( \
    ...
    partition_list_dropoff_year_month = [linha["dropoff_time_year_month"] for linha in df_gold_groupping.select("dropoff_time_year_month").distinct().orderBy("dropoff_time_year_month").collect()]

    print(f"  -> [ESCRITA] Salvando partição/ões {partition_list_dropoff_year_month} na tabela Gold: {GOLD_TABLE_METADATA['analysis_per_hour_per_month_full']['table']}")

    formatted_partitions = ", ".join([f"'{p}'" for p in partition_list_dropoff_year_month])

    save_as_table_delta(df_final_gold_per_hour_per_month, "overwrite", GOLD_TABLE_METADATA['analysis_per_hour_per_month_full']['table'], "dropoff_time_year_month", formatted_partitions)

def create_table_gold_analysis_per_month_per_color(spark, df):
    """
    ...
    df_gold_groupping = df.groupBy("taxi_color", "dropoff_time_year_month")\
        .agg( \
            F.count("*").alias("total_trips"),
            F.round(F.sum("total_amount"),2).alias("total_amount_sum"),
            F.round(F.avg("total_amount"),2).alias("avg_amount"),
    ...
    print(f"  -> [ESCRITA] Salvando partição/ões {partition_list_dropoff_year_month} na tabela Gold: {GOLD_TABLE_METADATA['analysis_per_month_per_color']['table']}")

    formatted_partitions = ", ".join([f"'{p}'" for p in partition_list_dropoff_year_month])

    save_as_table_delta(df_final_gold_per_month, "overwrite", GOLD_TABLE_METADATA['analysis_per_month_per_color']['table'], "dropoff_time_year_month", formatted_partitions)

# ==============================================================================
# ORQUESTRAÇÃO: SILVER TO GOLD
# ==============================================================================
def process_silver_to_gold(spark, particoes: list):
    """
    ...
    create_table_gold_full_data(spark, df_initial)
    create_table_gold_analysis_per_hour_per_month_full(spark, df_initial)
    create_table_gold_analysis_per_month_per_color(spark, df_initial)
